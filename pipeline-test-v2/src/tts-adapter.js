...
const { ffmpegBin, ffprobeBin } = require('./lib/ffmpeg-path');
...
function measureDurationSeconds(audioPath) {
  ...
  if (r.status !== 0) {
    throw new Error(`ffprobe failed (${r.status}): ${(r.stderr || '').trim() || r.signal}`);
  }
...
function synthesizeEspeak(text, outputPath, opts = {}) {
  ...
  const enc = spawnSync(
    ffmpeg,
    ['-y', '-loglevel', 'error', '-i', tmpWav, '-codec:a', 'libmp3lame', '-b:a', '128k', outputPath],
    { stdio: 'pipe', windowsHide: true }
  );
  if (enc.status !== 0) {
    throw new Error(`ffmpeg mp3 encode failed: ${(enc.stderr || '').toString().slice(0, 400)}`);
  }
...
function concatMp3s(chunkPaths, outputPath) {
  ...
  const r = spawnSync(
    ffmpeg,
    ['-y', '-loglevel', 'error', '-f', 'concat', '-safe', '0', '-i', listPath, '-c', 'copy', outputPath],
    { stdio: 'pipe', windowsHide: true }
  );
  if (r.status !== 0) {
    throw new Error(`ffmpeg concat failed: ${(r.stderr || '').toString().slice(0, 400)}`);
  }
